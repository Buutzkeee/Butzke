...
export class HomePage {
  ...
  _ebooks() {
    ...
    <div class="ebook-card card reveal delay-${i + 1}" role="button" onclick="Router.go('/ebook/${e.slug}')" style="display: flex; flex-direction: column; height: 100%; position: relative;">
      ${e.badge ? `<span class="badge badge-gold" style="position: absolute; top: 16px; right: 16px; z-index: 2; font-weight: 700; font-size: 0.7rem; letter-spacing: 1px; ${e.badge === 'LANÇAMENTO' ? 'background: linear-gradient(135deg, #d4a017, #8b6508); color: #000;' : ''}">${e.badge}</span>` : ''}
      <div class="ebook-card-top" style="justify-content:center; margin-bottom: 20px; height: 300px; display: flex; align-items: center;">
      ...
      <h3 style="font-size:1.15rem; letter-spacing:1px; margin-bottom:6px; min-height: 48px; display: flex; align-items: flex-start;">${e.title}</h3>
      <p class="ebook-sub" style="color: var(--gold); font-size: 0.75rem; font-weight: 700; text-transform: uppercase; letter-spacing: 1px; margin-bottom: 12px; flex-grow: 1;">${e.subtitle}</p>
      ${e.priceTo ? `
      <div style="margin-bottom: 16px; text-align: center;">
        ${e.priceFrom ? `<span style="font-size: 0.8rem; text-decoration: line-through; color: var(--text-faint); margin-right: 6px;">De R$ ${e.priceFrom.toFixed(2).replace('.', ',')}</span>` : ''}
        <span style="font-size: 1.25rem; font-family: var(--font-title); color: var(--gold); font-weight: 700;">R$ ${e.priceTo.toFixed(2).replace('.', ',')}</span>
      </div>` : ''}
